...
Both authentication approaches are supported with this library and signed JWT auth is the default. To use the API key approach you can supply the `--use_api_key_auth` command line switch, along with an API key from the NHS developer hub. For example:
