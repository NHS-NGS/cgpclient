...
from cgpclient.client import CGPClient, GenomicFiles


def parse_args(args: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fetch details of genomic files associated with an "
            "NGIS referral ID using the GDAM API in the NHS APIM"
        )
    )
    parser.add_argument(
        "-r",
    ...
    parser.add_argument(
        "-o", "--output_file", type=Path, help="Output file (default STDOUT)"
    )
    parser.add_argument(
        "-pp",
        "--pretty_print",
...
def main(cmdline_args: list[str]) -> GenomicFiles:
    ...
    with (
        ...
        print(
            files.model_dump_json(indent=4 if args.pretty_print else None), file=output
        )
